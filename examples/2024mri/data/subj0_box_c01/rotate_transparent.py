...
#def get_subobject(root, filter_function):
#    for obj in root:
...
def experiment(file_base, nrotate, out, begin=0, end=360, znorm=0.2, uznorm=0.9):
    ...
    for i, deg_ang in enumerate(deg_angles):
        ...
        for plot_field in root.iter("Object"):
            change = False
            if plot_field.get("name") == "PseudocolorAttributes":
                for f in plot_field.iter("Field"):
                    if f.get("name") == "opacityType" and f.text == "Ramp":
                        change = True
                if change:
                    for f in plot_field.iter("Field"):
                        if f.get("name") == "opacity":
                            f.text = str(0.1+0.3*4*h*(1-h))#np.sin(2*ang)*0.5+0.5)
                            print(f.get("name"), f.text)
