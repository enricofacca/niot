...
def explicit_expression(x,y,z):
    return (3 * conditional(x<1.0/nx,1,0)*conditional(y<1.0/ny,1,0)
            + 2 * conditional(x<1.0/nx,1,0)*conditional(z<1.0/nz,1,0)
            + 1 * conditional(y<1.0/ny,1,0)*conditional(z<1.0/nz,1,0) )
...
def test_3d_numpy_simplicial_mesh(mesh, example):
   # get number of cells
...
def test_3d_numpy_cartesian_mesh(mesh, example):
    # get number of cells
    if mesh.extruded:
...
def test_coordinates(mesh, lengths):   
...
test_3d_numpy_simplicial_mesh(mesh, example)
test_coordinates(mesh, lengths)

# create cartesian mesh
cartesian_mesh = i2d.build_mesh_from_numpy(example, mesh_type='cartesian',lengths=lengths)
test_3d_numpy_cartesian_mesh(cartesian_mesh, example)
test_coordinates(cartesian_mesh, lengths)
